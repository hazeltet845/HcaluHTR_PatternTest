...
event_binedges = np.arange(0,3025,25)
...
for k in range(1,event_binedges.shape[0]):
    ...
    command = f"cmsRun HcalDigiToRawPatTest_cfg.py -i {input_file} -s {binedge_0} -n 25 -g '140X_mcRun3_2024_realistic_v26' -e 'Run3_2025' -o '{input_tag}_offload_{binedge_0}-{binedge_1}'"

    os.system(command)
    print(f"Offload complete for {binedge_0}-{binedge_1}")

    command = f"python3 genPattern.py -o {input_tag}_{binedge_0}-{binedge_1} -i {input_tag}_offload_{binedge_0}-{binedge_1} --all"
