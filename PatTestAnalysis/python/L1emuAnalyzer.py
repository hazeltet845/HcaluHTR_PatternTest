...
param_LLP1 = [-1.09,1.35]
param_LLP2 = [0.04,0.74]
              #eta , phi
...
def L1Analyzer(infilePath,nEvents,emu,mc,print_all):
    ...
    h1_Et = ROOT.TH1F("h1_Et", ";E_{T};Entries", 1024, 0, 1024)
    h2_Et = ROOT.TH1F("h2_Et", ";E_{T};Entries", 1024, 0, 1024)
    h3_Et = ROOT.TH1F("h3_Et", ";E_{T};Entries", 1024, 0, 1024)
    
    if(int(runNum) == 390757):
        h3_2d = ROOT.TH2D("h1_2d", ";#eta;#phi",50,-0.2,1.2,50,-1.1,2.2)#280,628
    elif(int(runNum) == 390758):
        h3_2d = ROOT.TH2D("h1_2d", ";#eta;#phi",50,1.05,1.12,50,-0.9,-0.7)
    else:
        h3_2d = ROOT.TH2D("h1_2d", ";#eta;#phi",50,-1.4,1.4,50,-np.pi,np.pi)
    ...
    for i in range(nEvents):
        ...
        if (np.any(trigDecList) or mc):
            ...
            for k in range(int(nJets)):
                ...
                if((abs(eta-param_LLP1[0])< 0.01) and (abs(phi-param_LLP1[1])< 0.01) and hwqual == 1):
                    LLP = 1
                    et_LLP = et
                elif((abs(eta-param_LLP2[0])< 0.01) and (abs(phi-param_LLP2[1])< 0.01) and hwqual == 1):
                    LLP = 2
                    et_LLP = et
                elif(hwqual == 1):
                    LLP = 3
                ...
                if(print_all): print(f"  Jet {k}: Et = {round(et,1)}, Eta = {round(eta,2)}, Phi = {round(phi,2)}, Qual = {hwqual}")
    ...
    if(not mc):
        ...
        h3_2d.SetMarkerSize(0.65)

        h3_2d.Draw("COLZ TEXT")

        cmsLabel(runNum)
        c2.SaveAs(f"hist_2D_eta_phi_run{runNum}.png")
        del c2
# ------------------------------------------------------------------------------
def main():
